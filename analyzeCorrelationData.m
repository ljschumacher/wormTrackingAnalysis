...
for wormnum = wormnums
    ...
    for strainCtr = 1:nStrains
        %% load data
        if dataset == 1
            [phaseFrames,filenames,~] = xlsread(['datalists/' strains{strainCtr} '_' wormnum{1} '_list.xlsx'],1,'A1:E15','basic');
        elseif dataset == 2
            [phaseFrames,filenames,~] = xlsread(['datalists/' strains{strainCtr} '_' wormnum{1} '_g_list.xlsx'],1,'A1:E15','basic');
        end
