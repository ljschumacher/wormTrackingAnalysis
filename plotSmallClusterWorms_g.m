...
for numCtr = 1:length(wormnums)
    ...
    for strainCtr = 1:length(strains)
        ...
        for fileCtr=1:numFiles
            ...
            trajData.clusterskelfiltered = trajData.skelfiltered&...
            ...
            % plot sample data
            if length(unique(trajData.frame_number(trajData.clusterskelfiltered)))<numFramesSampled
                warning(['Not enough frames to plot for ' filename ])
            else
                smallClusterTrajFig = figure;
                framesAnalyzed = randsample(unique(trajData.frame_number(trajData.clusterskelfiltered)),numFramesSampled);
                for frameCtr = 1:numFramesSampled
                    frame=framesAnalyzed(frameCtr);
                    subplot(2,5,frameCtr)
                    % plot central green worm (in blue)
                    frameIdcs_worm = find(trajData.frame_number==frame&trajData.clusterskelfiltered);
                ...
                frameLogIdcs_pharynx = trajData.frame_number==frame&trajData.skelfiltered;
