function [] = analyzeReversalPharynxBBtest(dataset,phase,wormnum,useJoinedTraj)
...
%         end
        ...
        BBdensitybins = bayesianBlockBinEdges(knndensityAtRev(~isnan(knndensityAtRev(:,k)),k));
        % correct for (likely erronous) small density estimates
        BBdensitybins(1) = 0;
        BBdensitybins(BBdensitybins>0&BBdensitybins<k/7.5^2) = [];
        % subsample to estimate variability
        revRateSubsamples = subsampleRevFreq(knndensityAtRev(:,k),knndensity(:,k),...
            BBdensitybins,signedSpeeds,frameRate,minSpeedPerFrame,numSubSamples,sampleSizeFraction,sampleReplacement);
        %%% careful: last line uses frameRate of last files, only works if all frameRates are consistent
        plotbins = BBdensitybins(2:end) - diff(BBdensitybins)./2; % convert from bin edges to centers
        revRates = mean(revRateSubsamples,1);
        revRateErr = std(revRateSubsamples,0,1).*samplingCorrectionFactor;
        [~, mindensityIdx] = min(abs(BBdensitybins));
...
%             revFig.Children.YLim = [0 3];
            ...
            figurename = ['figures/reversals/phaseSpecific/reversals-densityBB_knn' num2str(k) ...
        ...
        BBdensitybins = bayesianBlockBinEdges(knndensityAtFwd(~isnan(knndensityAtFwd(:,k)),k));
        % correct for (likely erronous) small density estimates
        BBdensitybins(1) = 0;
        BBdensitybins(BBdensitybins>0&BBdensitybins<k/7.5^2) = [];
        % subsample to estimate variability
...
%             fwdFig.Children.YLim = [0 3];
            ...
            figurename = ['figures/reversals/phaseSpecific/forwards-densityBB_knn' num2str(k) ...
                '_pharynx_' wormnum '_' phase '_data' num2str(dataset)];
        ...
        BBdensitybins = bayesianBlockBinEdges(knndensityEitherRevFwd(~isnan(knndensityEitherRevFwd)));
        % correct for (likely erronous) small density estimates
        BBdensitybins(1) = 0;
        BBdensitybins(BBdensitybins>0&BBdensitybins<k/7.5^2) = [];
        % subsample to estimate variability
        revfwdRateSubsamples = subsampleRevFreq(knndensityEitherRevFwd,knndensity(:,k),...
            BBdensitybins,abs(signedSpeeds),frameRate,minSpeedPerFrame,numSubSamples,sampleSizeFraction,sampleReplacement);
        %%% careful: last line uses frameRate of last files, only works if all frameRates are consistent
        plotbins = BBdensitybins(2:end) - diff(BBdensitybins)./2; % convert from bin edges to centers
...
%             revfwdFig.Children.YLim = [0 3];
            revfwdFig.Children.XLim(2) = 10;
            revfwdFig.Children.Box = 'on';
            figurename = ['figures/reversals/phaseSpecific/reorient-densityBB_knn' num2str(k) ...
                '_pharynx_' wormnum '_' phase '_data' num2str(dataset)];
